    ...
    const draw = (now: number) => {
      ...
      for (let index = 0; index < luminanceField.length; index++) {
        ...
        luminanceField[index] =
          luminance <= threshold ? 0 : (luminance - threshold) / Math.max(0.001, 1 - threshold);
      }
...
export function AsciiImage(props: AsciiPresetProps) {
  return <AsciiEffect variant="image" {...props} />;
}

export function AsciiFlow(props: AsciiPresetProps) {
  return <AsciiEffect variant="flow" {...props} />;
}

export function AsciiGlitch(props: AsciiPresetProps) {
  return <AsciiEffect variant="glitch" {...props} />;
}
